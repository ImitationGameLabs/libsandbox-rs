...
impl ResourceBuilder {
    ...
    pub fn build(self) -> Result<ResourceConfig> {
        ...
        if let Some(limit) = self.config.memory_limit {
            if limit == 0 {
                return Err(SandboxError::Config(
                    "memory_limit must be greater than 0".into(),
                ));
            }
        }
        if let Some(limit) = self.config.cpu_limit {
            if limit <= 0.0 {
                return Err(SandboxError::Config(
                    "cpu_limit must be greater than 0".into(),
                ));
            }
        }
        if let Some(limit) = self.config.wall_time_limit {
            if limit.is_zero() {
                return Err(SandboxError::Config(
                    "wall_time_limit must be greater than 0".into(),
                ));
            }
        }
        if let Some(limit) = self.config.cpu_time_limit {
            if limit.is_zero() {
                return Err(SandboxError::Config(
                    "cpu_time_limit must be greater than 0".into(),
                ));
            }
        }
